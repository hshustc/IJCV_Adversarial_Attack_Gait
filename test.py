...
print('#######################################')
time = datetime.now()
eval_feature_pkl = '{}_{}set_{}_{}eval_feature.pkl'.format('_'.join(config['dataset']), config['test_set'], config['save_prefix'], os.path.split(config['ckp_prefix'])[-1])
if config['resume'] and os.path.exists(eval_feature_pkl):
    ...
    data = m.transform(config['test_set'], batch_size=config['batch_size'][0], feat_idx=config['feat_idx'])
